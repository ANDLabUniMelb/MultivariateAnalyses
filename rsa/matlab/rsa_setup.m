function [stim_path, roi_path, fig_path, results_path, subids, stim, masks, rounds, trials_lookup, RDMs] = rsa_setup(run_condition, smooth)
    ...
    [cosmo_dir, ~, ~] = fileparts(script_dir);
    stim_path = fullfile(project_dir, 'data', [run_condition, smooth]); % Modified to use run_condition
    roi_path = fullfile(project_dir, 'data', 'rois_final');
    addpath(genpath(fullfile(cosmo_dir, 'CoSMoMVPA-master')));

    % Adjust the figure and results saving path to include dataset type
    fig_path = fullfile(project_dir, 'scripts', 'figures', ['figures_', run_condition, smooth, '_', date]);
    results_path = fullfile(project_dir, 'scripts', 'results', ['results_', run_condition, smooth, '_', date]);
    ...
    stim = {'cat', 'goose', 'lion', 'grizzly', 'fist', 'stick', 'gun', 'grenade'};
    masks = {'amy_z10_subjspace_bin.nii.gz', 'insula_z7_subjspace_bin.nii.gz', 'vmpfc_neurovault_subjspace_bin.nii.gz',  'Win_vs_lose_group_bin.nii.gz', 'mask_6mm_antvmPFC_Qi.nii.gz'};
    % 'amy_z10_subjspace_bin.nii.gz', 'insula_z7_subjspace_bin.nii.gz', 
    % 'thresh_safetyPMincrease_group_bin.nii.gz', 
    ...
    RDMs = zeros(length(stim), length(stim), length(subids), length(masks));