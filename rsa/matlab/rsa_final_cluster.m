function rsa_final_cluster(run_condition, smooth)
% run_condition: 'first_zstat', 'second_zstat', 'test'
% smooth: '', '4mm', '6mm'
    if smooth ~= ""
        smooth = ['_', smooth];
    end
    ...
    % mean_activation = round_means(stim_path, roi_path, subids, stim, masks, rounds, trials_lookup);
    for mask_idx = 1:length(masks)
                    ...
                    for t = 1:num_trials
                        ...
                        if isfile(file_path)
                            try
                                ...
                                ds_samples = ds_trial.samples'; % Append samples to dat_stim
                                dat_stim(:, end+1) = ds_samples; % - mean_activation{sub, mask_idx, r};
                                counter = counter + 1;
                            catch ME
        ...
        imagesc(avg_RDM, [-1, 1]); % Plots the RDM with scale set from 0 to 1 for clarity
