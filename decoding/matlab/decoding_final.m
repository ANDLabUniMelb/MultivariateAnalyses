...
% Function to balance classes by undersampling
% balance_classes = @(tbl) [tbl(tbl.label == min(tbl.label), :); tbl(randsample(find(tbl.label == max(tbl.label)), min(sum(tbl.label == min(tbl.label)), sum(tbl.label == max(tbl.label)))), :)];
...
for i = 1:length(stim_subsets)
    ...
    % balancedTable = balance_classes(filteredTable);
