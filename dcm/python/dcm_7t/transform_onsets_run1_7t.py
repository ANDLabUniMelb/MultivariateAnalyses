...
base_output_dir = '/Users/joecussen/Documents/Jobs/unimelb/projects/dcm_7t/data/onsets/transformed'

# Define paths and parameters
round_num = 1
onsets_path = '/Users/joecussen/Documents/Jobs/unimelb/projects/dcm_7t/data/onsets/pm'
tr = 0.8

# Load and process onset files
dataframes, empty_files = [], []

reverse_animals = {0.2143: 'cat', 0.1429: 'goose', -0.1429: 'lion', -0.2143: 'grizzly'}
reverse_non_animals = {0.2143: 'grenade', 0.1429: 'gun', -0.1429: 'stick', -0.2143: 'fist'}

# Create the new column based on the 'animals' flag:


for file_name in filter(lambda f: f.endswith(".txt"), os.listdir(onsets_path)):
    file_path = os.path.join(onsets_path, file_name)
    df = pd.read_csv(file_path, sep='\s+', header=None, names=['onset', 'duration', 'parametric_mod'])
    df['filename'] = file_name
    df['round'] = df['filename'].str.extract(r'run-(\d+)').astype(int)
    df['subject_id'] = df['filename'].str.extract(r'sub-(\d+)').astype(int)
    df['first_pres'] = (df['filename'].str.contains('FirstStim')).astype(int)
    df['animals'] = (df['filename'].str.contains('animals')).astype(int)
    df['stim'] = np.where(df['animals'] == 1,
                                df['parametric_mod'].map(reverse_animals),
                                df['parametric_mod'].map(reverse_non_animals))
...
onsets_df = onsets_df[onsets_df['round']==round_num]
...
task_df = onsets_df[['subject_id', 'onset', 'duration']].copy()
task_df['name'] = 'task'
...
for condition in conditions:
    cond_df = condition_df[['subject_id', 'onset', 'duration', condition]].copy()
